...
class CityUtil
{
    const PROVINCE_LEVEL_CITY_LIST = ['北京','天津','重庆','上海','香港','澳门'];
    ...
    const CITY_DISTRICT = [
        ...
        "阳泉市" => ["城区","矿区","郊区","平定县","盂县"],
        "长治市" => ["潞州区","上党区","襄垣县","屯留区","平顺县","黎城县","壶关县","长子县","武乡县","沁县","沁源县","潞城区"],
        ...
        "海西蒙古族藏族自治州" => ["格尔木市","德令哈市","海西蒙古族藏族自治州直辖","乌兰县","都兰县","天峻县","茫崖市"],
        ...
        "可克达拉市" => ["可克达拉市"],
        "昆玉市" => ["昆玉市"],
        "台北市" => ["北投区","松山区","大同区","文山区","信义区","内湖区","中正区","万华区","中山区","士林区","南港区","大安区"],
        "高雄市" => ["小港区","左营区","大寮区","那玛夏区","旗津区","弥陀区","燕巢区","大树区","阿莲区","楠梓区","鸟松区","苓雅区","桥头区","梓官区","美浓区","仁武区","凤山区","甲仙区","茄萣区","大社区","前镇区","茂林区","湖内区","六龟区","林园区","内门区","鼓山区","杉林区","三民区","前金区","冈山区","田寮区","桃源区","盐埕区","旗山区","永安区","路竹区","新兴区"],
        "新北市" => ["八里区","三峡区","新庄区","坪林区","莺歌区","汐止区","深坑区","芦洲区","三芝区","三重区","平溪区","中和区","永和区","泰山区","土城区","石碇区","五股区","林口区","双溪区","树林区","板桥区","新店区","乌来区","贡寮区","金山区","万里区","淡水区","瑞芳区","石门区"],
        "台中市" => ["东区","新社区","中区","南屯区","神冈区","沙鹿区","丰原区","大里区","龙井区","西区","南区","西屯区","太平区","北屯区","大雅区","石冈区","北区","大甲区","梧栖区","东势区","和平区","雾峰区","乌日区","后里区","潭子区","大肚区","外埔区","清水区","大安区"],
        "台南市" => ["安定区","大内区","东山区","下营区","山上区","永康区","新营区","白河区","盐水区","归仁区","龙崎区","关庙区","仁德区","学甲区","善化区","新市区","佳里区","新化区","西港区","麻豆区","安南区","后壁区","柳营区","玉井区","官田区","东区","六甲区","中西区","北区","楠西区","南化区","左镇区","南区","安平区","北门区","七股区","将军区"],
        "桃园市" => ["平镇市","杨梅市","八德市","桃园区","龟山乡","复兴乡","芦竹乡","观音乡","龙潭乡","大溪镇","大园乡","新屋乡","中坜市"],
        "基隆市" => ["基隆市"],
        "新竹市" => ["新竹市"],
        "嘉义市" => ["嘉义市"],
        "新竹县" => ["新竹县"],
        ...
        "澳门特别行政区" => ["花王堂区","望德堂区","风顺堂区","圣方济各堂区","嘉模堂区","大堂区","路氹填海区","花地玛堂区"]
    ];

    const IS_TRIM = false;

    public static function readBaiDuMapJson()
    {
        $str = file_get_contents('./bd.json');
        return self::json2Array($str);
    }

    public static function json2Array($str)
    {
    ...
    public static function cityToProvince()
    {
        $jsonData = self::readBaiDuMapJson();

        $provinceColumn = array_column($jsonData, 'province');
        $cityColumn = array_column($jsonData, 'city');
        $combineColumn = array_combine($cityColumn, $provinceColumn);

        $str = "";
        foreach ($combineColumn as $key => $item) {
        ...
        file_put_contents('./cityToProvince.txt', $str);
    }

    public static function cityToDistrict()
    {
        $jsonData = self::readBaiDuMapJson();
        $cityColumn = array_unique(array_column($jsonData, 'city'));
        $combine = [];
        ...
        foreach ($cityColumn as $city) {
            ...
            foreach ($jsonData as $item) {
                if ($item['city'] == $city) {
                    array_push($combine[$city], $item['district']);
                }
            }
        }
        ...
        foreach ($combine as $key => $item) {
            ...
            $tmp = "\"" . join("\",\"", $item) . "\"";
            $cityDistrictStr .= "\"" . $key . '" => [' . $tmp . '],' . "\n\t\t";
        }

        file_put_contents('./cityToDistrict.txt', $cityDistrictStr);
    }
     ...
     * @return string 首字母
     */
    public static function getFirstLetterByStr($str)
    {
        $s0 = mb_substr($str, 0, 3);
    ...
    public static function groupByInitials($data)
    {
        $letterArray = array_map(function ($item) {
            return array_merge($item, [
                'province_s'    => self::getFirstLetterByStr($item['province']),
                'city_s'        => self::getFirstLetterByStr($item['city']),
                'district_s'    => self::getFirstLetterByStr($item['district']),
            ]);
        }, $data);

        file_put_contents('letter.json', self::array2Json($letterArray));
    }

    //缺一个补充CITY_CODE的方法
    public static function getCityGroupByProvince()
    {
        $cityProvinceMap = self::CITY_PROVINCE;
        $provinceMap = [];
        if (!$provinceMap){
            foreach ($cityProvinceMap as $name => $info) {
                $provinceMap[$info[0]][] = [$name, $info[0]];
            }
        }
    ...
    {
        return self::CITY_DISTRICT[$city] ?: [];
    }

    public static function optionProvinceCityDistrict()
    {
        $data = [];
        $provinceCity = self::getCityGroupByProvince();
        foreach ($provinceCity as $province => $city){
            $options = [];
    ...
    public static function trimCharacter($str, $trim)
    {
        if (!$trim) {
            return $str;
        }

        $lastCharacter = mb_substr($str, -1, 1, 'UTF-8');
        if (in_array($lastCharacter, explode(',', $trim))) {
    ...
    public static function insertAllProvince()
    {
        $data = self::readBaiDuMapJson();
        $provinceArr = array_unique(array_column($data, 'province'));
        ...
        foreach ($provinceArr as $item) {
            $item = self::trimCharacter($item, '省,市');

            if (in_array($item, self::PROVINCE_LEVEL_CITY_LIST)) {
                continue;
            }
        ...
        //$sql = "INSERT INTO {$tableName} (`province`,`city`,`district`,`sn`,`created_at`,`updated_at`) VALUES {$result};";
        //file_put_contents('./province.sql', $sql);
        file_put_contents('./p.json', self::array2Json($tmp));
    }

    public static function insertAllProvinceCity()
    {
        $data = self::readBaiDuMapJson();
        ...
        foreach ($data as $item) {
            $province = self::trimCharacter($item['province'], '省,市');
            $city = self::trimCharacter($item['city'], '市');
            ...
            $tmp[$str] = ['all'=> "{$item['province']}{$item['city']}", 'mysql' => $str];

            $result[] = "('{$province}','{$city}','','','{$date}','{$date}')";
        }

        //$result = implode(',', $result);
        //$sql = "INSERT INTO {$tableName} (`province`,`city`,`district`,`sn`,`created_at`,`updated_at`) VALUES {$result};";
        //file_put_contents('./provinceCity.sql', $sql);
        file_put_contents('./p_c.json', self::array2Json($tmp));
    }

    public static function insertAllProvinceCityDistrict()
    {
        $data = self::readBaiDuMapJson();
        ...
        foreach ($data as $item) {
            ...
            $str = "{$province}_{$city}_{$district}";

            if (in_array($str, $tmp)) {
                continue;
            }

            $tmp[$str] = ['all'=> "{$item['province']}{$item['city']}{$item['district']}", 'mysql' => $str];

            $result[] = "('{$province}','{$city}','{$district}','','{$date}','{$date}')";
        }

        //$result = implode(',', $result);
        //$sql = "INSERT INTO {$tableName} (`province`,`city`,`district`,`sn`,`created_at`,`updated_at`) VALUES {$result};";
        //file_put_contents('./provinceCityDistrict.sql', $sql);
        file_put_contents('./p_c_d.json', self::array2Json($tmp));
    }
}

//CityUtil::insertAllProvince();
//CityUtil::insertAllProvinceCity();
CityUtil::insertAllProvinceCityDistrict();