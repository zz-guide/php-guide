 ...
 * Class MercatorUtil
 */
class MercatorUtilBK
{
    const GCJ02LL = 1;      //国测局（GCJ02）坐标;
    const BD09LL = 2;       //百度经纬度坐标
    const WGS84LL = 3;      //WGS84坐标,GPS
    const GCJ02MC = 4;
    const bBD09MC = 5;       //百度米制经纬度坐标
    const WGS84MC = 6;

    private $coordTypeMapping = ['', 'gcj02ll', 'bd09ll', 'wgs84ll', 'gcj02mc', 'bd09mc', 'wgs84mc'];
    ...
    {
        $this->from = $this->coordTypeMapping[$from];
        $this->to = $this->coordTypeMapping[$to];

    }

    private function conv($x, $y, $factor)
    {
        ...
        $toX *= ($x < 0? -1: 1);
        $toY *= ($y < 0? -1: 1);
        return array('x'=>$toX, 'y'=>$toY);
    }

    //经纬度转墨卡托坐标
    private function ll2mc($x, $y)
    {
        if ($x > 180.0)
            $x = 180.0;
        else if ($x < -180.0)
        ...
        return self::conv($x, $y, $factor);
    }

    //墨卡托坐标转经纬度
    private function mc2ll($x, $y)
    {
        if ($x > 20037508.342)
            $x = 20037508.342;
    ...
    public function convert($x, $y)
    {
        ...
        $fromType = $this->from;
        $toType = $this->to;
        $fromLen = strlen($fromType);
        $toLen = strlen($toType);

        if (substr($fromType, 0, $fromLen-2) == substr($toType, 0, $toLen-2)) {
            if (self::endsWith($fromType, 'll') && self::endsWith($toType, 'mc'))
                return self::ll2mc($x, $y);
            else return self::mc2ll($x, $y);
        }

        $newx = 0;
        $newy = 0;
        if (self::endsWith($fromType, 'll') && ($toType == 'bd09mc' || $toType == 'bd09ll')) {
            //$ret = coordtrans($fromType, 'bd09mc', $x, $y, $newx, $newy);
            if ($toType == 'bd09ll')
                return self::mc2ll($newx, $newy);
        }
